...
const ButtonProject = ({
  ...
  return (
    <div
      className={`flex justify-between flex-col md:flex-row gap-4 lg:gap-3 mt-4 flex-wrap ${width}`}
    >
      {demo != null ? (
        <a
      ...
      <div className="flex w-full gap-3">
        {github != null && (
          <a
            ...
            onClick={(e) => e.stopPropagation()}
            target="_blank"
            className={`flex justify-center items-center relative bg-secondary hover:text-white isolation-auto z-10 px-12 font-bold text-white rounded-[10px] transition-all duration-300 active:scale-90 active:shadow-[7px_5px_50px_-10px_#A364FF] w-full text-nowrap border-primary border-2  ${
              sm
                ? `text-[12px] lg:text-[14px] py-2`
                : "text-[12px] lg:text-[16px] py-3"
